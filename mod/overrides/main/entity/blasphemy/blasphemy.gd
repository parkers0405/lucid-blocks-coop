...
func _ready() -> void :
    ...
    for i in range(leg_count):
        ...
        d.mesh = SphereMesh.new()
        d.mesh.radius = 0.025
        d.mesh.height = 0.05

        var new_ray: RayCast3D = RayCast3D.new()
...
func _on_body_entered_attack(body: Node3D) -> void :
    if body == self:
        return
    if len(near_entities) > max_targets:
        return
    if not body in near_entities:
        near_entities.append(body)
    if len(near_entities) > 0 and has_node("%AttackTimer") and %AttackTimer.is_stopped():
        _on_attack_timeout()
        %AttackTimer.start(attack_timeout)


func _on_body_exited_attack(body: Node3D) -> void :
    if body in near_entities:
        near_entities.erase(body)
    if len(near_entities) == 0 and is_inside_tree() and has_node("%AttackTimer"):
        %AttackTimer.stop()
...
func leg_process() -> Array[float]:
    var total_displacement: float = 0
    ...
    for i in range(len(legs)):
        if legs[i].on_ground:
            ...
            if not legs[i].is_animating and (legs[i].foot_position.distance_to(leg_targets[i].global_position) > reset_distance):
                ...
                if ( not legs[i - 1].is_animating or legs[i - 1].is_steady) and ( not legs[(i + 1) % len(legs)].is_animating or legs[(i + 1) % len(legs)].is_steady):
            ...
            legs[i].reset()
    return [total_displacement, applying_force]
...
func spring_process(delta: float, total_displacement: float, applying_force: float) -> void :
    ...
    unstick = lerp(unstick, 0.0, clamp(delta * restick_speed, 0.0, 1.0))
    unstick = clamp(unstick, 0.0, 1.0)
...
func _physics_process(delta: float) -> void :
    ...
    movement_velocity = lerp(movement_velocity, stuck_impulse + desired_velocity, min(1.0, delta * ground_accel))
    movement_velocity.y = vertical_movement_velocity

    stuck_impulse = lerp(stuck_impulse, Vector3(), min(1.0, delta * ground_accel))
...
func _on_attacked(attacker) -> void :
    if is_session_player_entity(attacker):

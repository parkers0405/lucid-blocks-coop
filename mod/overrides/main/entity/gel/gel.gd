...
func _get_target_position() -> Vector3:
    return get_session_target_position(global_position)
...
func _on_jump_timeout() -> void :
    ...
    if not is_instance_valid(bothersome):
        if _has_session_player_target():
            var displacement: Vector3 = _get_target_position() - global_position
...
func update_bothersome_target() -> void :
    ...
    if is_instance_valid(nearest_player):
        bothersome = nearest_player
    elif is_instance_valid(nearest):
        bothersome = nearest
    elif bothersome == null:

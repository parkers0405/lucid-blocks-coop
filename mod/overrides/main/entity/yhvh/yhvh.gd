...
func _on_area_entered_attack(area: Area3D) -> void :
    ...
    entity.knockback_velocity += 0.45 * arm_movement_velocity + horizontal_kb * arm_knockback_strength
    entity.knockback_velocity.y += (arm_knockback_strength * entity.jump_modifier * (0.5 if not entity.is_on_floor() else 1.0))
    ...
    if entity.has_node("%Bleed"):
        var target_to_attacker = (global_position - entity.global_position).normalized()
...
func _on_comet_timeout() -> void :
    ...
    new_comet.shoot(offset + get_session_target_position(Ref.player.global_position), direction)
...
func _on_spawn_timeout() -> void :
    ...
    %SpawnCheck.global_position = offset + get_session_target_position(Ref.player.global_position)
...
func _on_laser_timeout() -> void :
    ...
    if state == CALM_STORM or dead or disabled or shooting_laser:
        return
    var general_direction: Vector3 = (%Laser.global_position.direction_to(get_session_target_head_position(Ref.player.head.global_position)) + 0.1 * Vector3(randf_range(-1, 1), randf_range(-1, 1), randf_range(-1, 1))).normalized()
    %HitCast.target_position = Vector3(0, 0, - laser_warning_max_distance)

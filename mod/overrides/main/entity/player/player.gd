...
func die() -> void :
    ...
    if Ref.coop_manager != null and Ref.coop_manager.is_death_override_active():
        print("[lucid-blocks-coop] Player.die intercepted for coop")
    ...
    if not is_interacting():
        if attack_pressed and Input.is_action_just_pressed("attack"):
            ...
            if "target" in data:
                attack_used = true
                %WhiffPlayer.play()
                %Attack.attack(data.target, %InteractRayCast3D.get_collision_point())
                %PlayerHand.current_hand.hit()
...
func get_interact_data(skip_look: bool = false) -> Dictionary:
    ...
    if skip_look:
        if %LookRayCast3D.is_colliding():
            ...
            if is_instance_valid(collider):
                var look_entity: Entity = collider.owner as Entity
    ...
    for i in range(2):
        ...
        if %InteractRayCast3D.is_colliding():
            ...
            if collider.owner is Ball or collider.owner is Heart:
                data.ball_target = collider.owner
                non_block_position = data.interact_end
                non_block_collision = true
            elif collider.owner is Entity:
                data.target = collider.owner as Entity
                non_block_position = data.interact_end
                non_block_collision = true
            else:

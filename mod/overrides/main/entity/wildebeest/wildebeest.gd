...
func _get_target_position() -> Vector3:
    return get_session_target_position(bothersome.global_position if is_instance_valid(bothersome) else global_position)
...
func update_bothersome_target() -> void :
    ...
    for entity in bothersome_entities:
        if entity == attacker or (bothersome != attacker and ((is_session_player_entity(entity) and not bothersome is Bubblebear) or entity is Bubblebear)):
            bothersome = entity
...
func idle_process(delta: float) -> void :
    ...
    anger += trespass_anger * float(len(bothersome_entities) + (1 if _has_session_player_target() else 0)) * delta

    if anger > anger_threshold and (is_instance_valid(bothersome) or _has_session_player_target()):
        switch_state(CHASE)
...
func chase_process(delta: float) -> void :
    if not is_instance_valid(bothersome) and not _has_session_player_target():
        switch_state(IDLE)
        return
    ...
    var difference_vector: Vector3 = _get_target_position() - global_position

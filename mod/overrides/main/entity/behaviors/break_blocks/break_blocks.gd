...
func drop_item() -> void:
    ...
    if block.drop_item != null:
        var explicit_state: ItemState = ItemState.new()
...
func break_block_instant(block_position: Vector3i) -> void:
    ...
    var world_block: Block = Ref.world.get_block_type_at(block_position)
    if world_block == null:
        return
    if world_block.id == 0 and world_block.internal_name != "cutscene block":
        return

    active_position = block_position
    block = ItemMap.map(world_block.id)
    ...
    if entity == Ref.player and Ref.coop_manager != null:
        Ref.coop_manager.notify_local_world_state_dirty([block_position])
...
func break_block_start(block_position: Vector3i) -> void:
    ...
    var world_block: Block = Ref.world.get_block_type_at(block_position)
    if world_block == null or world_block.id == 0 or (world_block.unbreakable and not Ref.main.creative):
        return
    if breaking:
        break_block_stop()
...
func _process(delta: float) -> void:
    if not breaking or entity.disabled or not enabled:
        return

...
func _process(delta: float) -> void :
    ...
    if not is_instance_valid(target) or disabled or not is_session_position_loaded(global_position):
        return
    ...
    if state == IDLE:
        ...
        SpatialMath.look_at(self, target.head.global_position if is_instance_valid(target.head) else target.global_position)

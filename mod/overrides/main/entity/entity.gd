...
func _ready() -> void :
    if swarming:
        var source_scene_path: String = scene_file_path if scene_file_path != "" else str(get_meta("coop_source_scene_path", ""))
        ...
        for i in range(count):
            ...
            if source_scene_path != "":
                new_entity.set_meta("coop_source_scene_path", source_scene_path)
...
func _supports_multi_region_session_world() -> bool:
    return Ref.coop_manager != null and Ref.coop_manager.has_connected_remote_peers()
...
func _request_coop_player_death_intercept() -> bool:
    if not (self is Player and not dead and Ref.coop_manager != null and Ref.coop_manager.is_death_override_active()):
        return false
    if Ref.coop_manager.is_local_player_fake_dead():
        return true
...
func _is_client_synced_entity_copy() -> bool:
    return Ref.coop_manager != null and Ref.coop_manager.is_client_synced_entity(self)


func _can_apply_client_synced_authority_change() -> bool:
    return _is_client_synced_entity_copy() and Ref.coop_manager.is_client_entity_authoritative_change_active(self)


func _should_block_client_synced_side_effects() -> bool:
    return _is_client_synced_entity_copy() and not _can_apply_client_synced_authority_change()


func is_session_player_entity(entity) -> bool:
    return is_instance_valid(entity) and (entity == Ref.player or (Ref.coop_manager != null and Ref.coop_manager.is_remote_player_proxy(entity)))
...
func get_session_target_entity_position(default_position: Vector3, default_target = null) -> Vector3:
    var target = get_session_target_entity(default_target)
    if is_instance_valid(target):
        return target.global_position
    return get_session_target_position(default_position)


func get_session_target_entity_head_position(default_position: Vector3, default_target = null) -> Vector3:
    var target = get_session_target_entity(default_target)
    if is_instance_valid(target):
        if is_instance_valid(target.head):
            return target.head.global_position
        return target.global_position + Vector3(0, 1.45, 0)
    return get_session_target_head_position(default_position)


func get_remote_session_player_peer_id(entity) -> int:
    if not is_session_player_entity(entity) or entity == Ref.player or Ref.coop_manager == null:
        return -1
    return Ref.coop_manager.get_remote_player_proxy_peer_id(entity)


func add_session_player_plot_data(entity, key: String, amount: int) -> void:
    if not is_session_player_entity(entity):
        return
    if entity == Ref.player:
        Ref.plot_manager.set_plot_data(key, Ref.plot_manager.get_plot_data(key, 0) + amount)
        return
    var peer_id: int = get_remote_session_player_peer_id(entity)
    if peer_id > 1:
        Ref.coop_manager.send_plot_increment.rpc_id(peer_id, key, amount)


func reward_session_player_hate(entity, amount: int, chance: float = 1.0) -> void:
    if not is_session_player_entity(entity) or randf() >= chance:
        return
    if entity == Ref.player:
    ...
    var peer_id: int = get_remote_session_player_peer_id(entity)
    if peer_id > 1:
        Ref.coop_manager.send_hate_reward.rpc_id(peer_id, amount)


func unlock_session_player_achievement(entity, achievement_id: String) -> void:
    if not is_session_player_entity(entity):
        return
    if entity == Ref.player:
        Steamworks.set_achievement(achievement_id)
        return
    var peer_id: int = get_remote_session_player_peer_id(entity)
    if peer_id > 1:
        Ref.coop_manager.send_achievement_unlock.rpc_id(peer_id, achievement_id)
...
func attacked(attacker, damage: int) -> void :
    if _should_block_client_synced_side_effects():
        return
    if dead or disabled:
        return
...
func die() -> void :
    if _is_client_synced_entity_copy():
        if _should_block_client_synced_side_effects():
            return
        if dead:
            return
    ...
    elif is_session_player_entity(last_attacker):
        var proxy_peer_id: int = get_remote_session_player_peer_id(last_attacker)
        if proxy_peer_id > 1:
...
func preserve_save(file: SaveFile, uuid: String) -> void :
    ...
    var save_in_air: bool = in_air
    var save_position: Vector3 = global_position
    var save_movement_velocity: Vector3 = movement_velocity
    var save_gravity_velocity: Vector3 = gravity_velocity
    var save_knockback_velocity: Vector3 = knockback_velocity
    var save_rope_velocity: Vector3 = rope_velocity
    var saved_health: int = health
    if multidimensional and self == Ref.player and Ref.coop_manager != null and Ref.coop_manager.should_apply_local_fake_death_save_override():
        var fake_death_overrides: Dictionary = Ref.coop_manager.get_local_fake_death_save_overrides()
        save_in_air = bool(fake_death_overrides.get("in_air", false))
        save_position = fake_death_overrides.get("global_position", global_position)
        save_movement_velocity = fake_death_overrides.get("movement_velocity", Vector3.ZERO)
        save_gravity_velocity = fake_death_overrides.get("gravity_velocity", Vector3.ZERO)
        save_knockback_velocity = fake_death_overrides.get("knockback_velocity", Vector3.ZERO)
        save_rope_velocity = fake_death_overrides.get("rope_velocity", Vector3.ZERO)
        saved_health = int(fake_death_overrides.get("health", maxi(1, max_health)))
    ...
    file.set_data("node/%s/in_air" % uuid, save_in_air, false)
    file.set_data("node/%s/global_position" % uuid, save_position, false)
    file.set_data("node/%s/movement_velocity" % uuid, save_movement_velocity, false)
    file.set_data("node/%s/gravity_velocity" % uuid, save_gravity_velocity, false)
    file.set_data("node/%s/knockback_velocity" % uuid, save_knockback_velocity, false)
    file.set_data("node/%s/rope_velocity" % uuid, save_rope_velocity, false)
    file.set_data("node/%s/health" % uuid, saved_health, multidimensional)
...
func preserve_load(file: SaveFile, uuid: String) -> void :
    ...
    var loaded_health: int = file.get_data("node/%s/health" % uuid, max_health, multidimensional)
    if multidimensional and loaded_health <= 0:
        loaded_health = maxi(1, max_health)

...
func _ready() -> void:
    ...
    call_deferred("_bootstrap_native_patch")
...
func _apply_native_patch_config(config_path: String) -> void:
    ...
    if coop_native_patch.has_method("get_status"):
        print("[lucid-blocks-coop] Native patch status after config: %s" % str(coop_native_patch.call("get_status")))

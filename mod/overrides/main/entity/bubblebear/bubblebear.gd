...
func _on_attacked(attacker: Entity) -> void :
    ...
    elif attacker is Player and Ref.coop_manager != null and Ref.coop_manager.is_remote_player_proxy(attacker):
        Ref.coop_manager.send_plot_increment.rpc_id(Ref.coop_manager.get_remote_player_proxy_peer_id(attacker), "bubblebear_hits", 1)
    if state == PANIC:
        return
...
func die() -> void :
    ...
    elif panic_source is Player and Ref.coop_manager != null and Ref.coop_manager.is_remote_player_proxy(panic_source):
        if randf() < 0.2:
            Ref.coop_manager.send_hate_reward.rpc_id(Ref.coop_manager.get_remote_player_proxy_peer_id(panic_source), 1)
        Ref.coop_manager.send_achievement_unlock.rpc_id(Ref.coop_manager.get_remote_player_proxy_peer_id(panic_source), "BUBBLEBEAR_MASSACRE")
    super.die()

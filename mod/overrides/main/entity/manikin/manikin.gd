...
func _physics_process(delta: float) -> void :
    ...
    if not is_instance_valid(player) and _has_session_player_target() and state != CHASE:
        ...
        elif _has_session_player_target() and Ref.coop_manager != null:
            attack_target = Ref.coop_manager.get_nearest_session_player_entity(global_position, attack_target)

...
func _on_attacked(attacker) -> void :
    add_session_player_plot_data(attacker, "bubblebear_hits", 1)
    if state == PANIC:
        return
...
func die() -> void :
    ...
    reward_session_player_hate(panic_source, 1, 0.2)
    unlock_session_player_achievement(panic_source, "BUBBLEBEAR_MASSACRE")
    super.die()

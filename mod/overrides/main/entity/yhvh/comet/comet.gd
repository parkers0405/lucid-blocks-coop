...
func _physics_process(delta: float) -> void:
    ...
    var fallback_distance: float = global_position.distance_to(Ref.player.global_position) if is_instance_valid(Ref.player) else 0.0
    var nearest_player_distance: float = Ref.coop_manager.get_nearest_session_player_distance(global_position, fallback_distance) if Ref.coop_manager != null else fallback_distance
    if nearest_player_distance > 200.0:
        queue_free()

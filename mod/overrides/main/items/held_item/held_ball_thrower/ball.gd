...
func update_biome() -> void :
    ...
    gravity_scale *= biome.gravity_scale
...
func deal_damage(target: Entity) -> void :
    ...
    Attack.basic_attack_override_direction(entity_owner, target, damage, global_position, (global_position - target.global_position).normalized(), false)

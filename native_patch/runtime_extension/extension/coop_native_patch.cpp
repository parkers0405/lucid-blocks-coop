...
static constexpr MultiRegionHookSite MULTI_REGION_HOOK_SITES[] = {
    ...
    {"simulate_dynamic radius check B", 0x0005d8bc, 16, 0x0005d90b, 0x0005d910},
};
...
static void store_active_region_centers(const Array &centers) {
    uint32_t count = 0;
    for (int64_t i = 0; i < centers.size() && count < MAX_ACTIVE_REGION_CENTERS; i++) {
        Variant value = centers[i];
        if (value.get_type() != Variant::VECTOR3) {
            continue;
        }
        g_active_region_centers[count] = snap_center_to_chunk(static_cast<Vector3>(value));
        count++;
    }

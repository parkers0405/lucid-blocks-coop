...
func _ready() -> void:
    ...
    while _is_session_position_loaded(transform.origin.floor()) and Ref.world.is_block_solid_at(transform.origin.floor()):
...
func _is_session_player_body(body: Node3D) -> bool:
    return _is_session_player_entity(body)
...
func _on_attack_timeout() -> void:
    if dead or disabled or state != CHASE:
        return
...
func thrust(target_direction: Vector3) -> void:
    ...
    tween.tween_property(%Core, "position", Vector3.ZERO, 0.3)
    %WhiffPlayer3D.play()


func capture_block() -> Block:
    var block_position: Vector3 = global_position.floor() - Vector3(0, 1, 0)
    if not _is_session_position_loaded(block_position) or not Ref.world.is_block_solid_at(block_position):
...
func _physics_process(delta: float) -> void:
    ...
    if state == CHASE:
        ...
        movement_velocity = lerp(movement_velocity, desired_velocity, minf(1.0, delta * ground_accel))
        ...
        while _is_session_position_loaded(global_position) and Ref.world.is_block_solid_at(global_position):
            global_position.y += 1

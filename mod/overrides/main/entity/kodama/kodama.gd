    ...
    set(val):
        ...
        if state == IDLE and anger >= anger_threshold:
            ...
            if not is_instance_valid(attack_target):
                attack_target = _get_session_attack_target()
...
func _has_session_player_target() -> bool:
    return _use_server_session_targeting() and is_session_target_within(maxf(process_distance, break_trigger_distance + 2.0))


func _get_session_attack_target():
    var fallback = attack_target if is_instance_valid(attack_target) else Ref.player
    return get_session_target_entity(fallback)
...
func _get_target_head_position() -> Vector3:
    ...
    if is_instance_valid(target) and is_instance_valid(target.head):
        fallback = target.head.global_position
...
func _on_block_broken(break_position: Vector3i) -> void :
    if (is_instance_valid(player) or _has_session_player_target()) and break_position.distance_to(global_position) <= break_trigger_distance:
        anger += randf_range(break_anger_min, break_anger_max)


func _on_block_placed(place_position: Vector3) -> void :
    if (is_instance_valid(player) or _has_session_player_target()) and place_position.distance_to(global_position) <= break_trigger_distance:
        anger += randf_range(break_anger_min, break_anger_max)
...
func _physics_process(delta: float) -> void :
    ...
    var next_direction: Vector3 = (Vector3(sin(time) * 0.2 + cos(time + time) * 0.3, -0.01 + 0.05 * sin(time) + 0.1 * cos(time), 0.2 * sin(time + time) + 0.2 * cos(time)).normalized())

    if state == HOSTILE:
        var target = _get_session_attack_target()
        if is_instance_valid(target):
            attack_target = target
            var target_head: Vector3 = _get_target_head_position()
            var to_target_direction: Vector3 = (target_head - %Core.global_position).normalized()
            ...
            if %AttackTimer.is_stopped() and target_head.distance_to(%Core.global_position) < attack_begin_distance:
                burst_attack()
